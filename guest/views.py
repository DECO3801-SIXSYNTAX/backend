...
def debug_decode_guest(request):
    ...
    try:
        ...
        event = payload.get("e")
        guest = payload.get("g")
        fresh_guest = get_guest(event, guest)  # <- repo call ke Firestore
        if not fresh_guest:
            return Response({"detail": "Guest not found"}, status=404)
        
        def gget(obj, key, default=None):
            return obj.get(key, default) if isinstance(obj, dict) else getattr(obj, key, default)

        already_checked = bool(gget(fresh_guest, "checkedIn"))
    ...
    try:
        ...
        already_checked = bool(gget(fresh_guest, "checked_in") or gget(fresh_guest, "is_checked_in"))
        checked_in_at = gget(fresh_guest, "checked_in_at")

        
        if already_checked:
            msg = "Guest already check-in."
            if checked_in_at:
                msg = f"Guest already check-in pada {checked_in_at}."
            return Response({"detail": msg, "payload": payload, "guest": fresh_guest}, status=409)

        # 4) Kalau belum → set True + timestamp
        patch = {"checked_in": True, "checked_in_at": timezone.now().isoformat()}
        updated_guest = partial_update_guest(event, guest, patch)
